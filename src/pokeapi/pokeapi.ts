...
export class PokeAPI {
    ...
    async fetchLocation(locationName?: string): Promise<LocationArea> {
        ...
        // Query cache
        let result: LocationArea | undefined = this.pokeCache.get(locationName);
        if (result !== undefined) {
            return result;
        }
    ...
    async fetchPokemon(pokemonName: string): Promise<PokemonResult> {
        if (!pokemonName) {
            pokemonName = "";
        }

        // Query cache
        let result: PokemonResult | undefined = this.pokeCache.get(pokemonName);
        if (result !== undefined) {
            return result;
        }
        ...
        try {
            ...
            result = (await response.json()) as PokemonResult;
            this.pokeCache.add(pokemonName, result);
            return result;
        } catch (error: any) {
            console.error(error.message);
            return Promise.reject();
